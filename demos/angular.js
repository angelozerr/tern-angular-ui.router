(function(mod) {
  ...
  function globalInclude(name) {
    ...
    var extProviders = infer.cx().parent._angular.providers;
    for (var i = 0; i < extProviders.length; i++) {
		var provider = extProviders[i];
	    if (provider.hasProp(name)) return provider.getProp(name);
	}
  }
  ...
  function postLoadDef(json) {
    ...
    var providers = defs && defs["!ng.providers"];
    if (providers) cx.parent._angular.providers.push(providers);
  }
